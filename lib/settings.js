...
const debug = require('util').debuglog('kraken/settings');
...
module.exports = function settings(app, options) {
    ...
    function mount(app, options, config) {
        ...
        const settings = config.get('express');
        const defaults = Object.keys(app.settings);
        ...
        if (options.inheritViews) {
            ...
            handler = function (name) {
                const idx = defaults.indexOf(name);
